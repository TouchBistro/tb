...
func New(projectName string, opts Options) (*Docker, error) {
...
// listContainers lists containers belonging to the project. If serviceNames is provided it will be used to filter
...
func (d *Docker) RemoveImages(ctx context.Context, imageSearches []ImageSearch) error {
	...
	for _, is := range imageSearches {
		if is.LocalBuild {
			...
			n := buildImageName(d.projectName, NormalizeName(is.Name))
