...
- run the build for specific branch in an iOS 12.3 iPad Air 2 simulator
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		s3Dir := filepath.Join(appName, runOpts.branch)
		...
		localBranchDir := filepath.Join(downloadDest, appName, runOpts.branch)
		...
		if len(localBuilds) == 1 {
			...
			log.Infof("Checking latest github sha for %s-%s", app.GitRepo, runOpts.branch)
			latestGitsha, err := git.GetBranchHeadSha(app.GitRepo, runOpts.branch)
			if err != nil {
				fatal.ExitErrf(err, "Failed getting branch head sha for %s-%s", app.GitRepo, runOpts.branch)
			}
			log.Infof("Latest github sha is %s", latestGitsha)
			if !strings.HasPrefix(s3BuildFilename, latestGitsha) {
				log.Warnf("sha of s3 build %s does not match latest github sha %s for branch %s", s3BuildFilename, latestGitsha, runOpts.branch)
			}
		...
		log.Infof("☑ Installed app %s on %s\n", app.BundleID, runOpts.deviceName)
		...
		if err != nil {
			fatal.ExitErrf(err, "☒ Failed to launch app %s on simulator %s", app.BundleID, runOpts.deviceName)
		}
