...
- run the build for specific branch in an iOS 12.3 iPad Air 2 simulator
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		storageProvider, err := storage.GetProvider(app.Storage.Provider)
		if err != nil {
			fatal.ExitErrf(err, "Failed getting storage provider %s", app.Storage.Provider)
		}

		s3Builds, err := storageProvider.ListObjectKeysByPrefix(app.Storage.Bucket, s3Dir)
		if err != nil {
		...
		if len(localBuilds) == 0 || refreshLocalBuild {
			...
			go func(successCh chan string, failedCh chan error) {
				err = storageProvider.DownloadObject(app.Storage.Bucket, pathToS3Tarball, dstPath)
				if err != nil {
